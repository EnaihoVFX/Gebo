...
export function ClipGrid({ clips, onAddClips, onRemoveClip, onDragStart }: ClipGridProps) {
  ...
  const getClipIcon = useCallback((clip: Clip) => {
    switch (clip.type) {
      case 'Video':
        return <Video className="w-6 h-6 text-editor-text-tertiary" />;
      case 'Audio':
        return <Music className="w-6 h-6 text-editor-text-tertiary" />;
      case 'Image':
        return <Image className="w-6 h-6 text-editor-text-tertiary" />;
      default:
        return <Video className="w-6 h-6 text-editor-text-tertiary" />;
    }
  }, []);
    ...
    if (clips.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-center">
          <div className="w-12 h-12 mx-auto mb-3 rounded-lg bg-editor-bg-glass-tertiary border border-editor-border-tertiary flex items-center justify-center backdrop-blur-xl">
            <Video className="w-6 h-6 text-editor-text-tertiary" />
          </div>
          <p className="text-sm text-editor-text-tertiary mb-4">No clips loaded</p>
          <button
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
            ...
            className="px-4 py-2 bg-editor-interactive-active text-white rounded-lg hover:bg-editor-interactive-hover transition-colors text-sm flex items-center gap-2 border border-editor-border-secondary"
          >
    ...
    return (
      <div className="grid grid-cols-2 gap-3">
        {clips.map((clip) => (
          <div
            key={clip.id}
            className={`group relative bg-editor-bg-glass-secondary rounded-lg border border-editor-border-tertiary overflow-hidden cursor-grab hover:border-editor-border-secondary transition-colors backdrop-blur-xl ${
          ...
          >
            ...
            <div className="relative w-full h-full">
              {clipThumbnails.has(clip.id) ? (
                ...
                <div className="w-full h-full bg-editor-bg-glass-tertiary flex items-center justify-center">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-editor-interactive-active"></div>
                </div>
              ) : (
                <div className="w-full h-full bg-editor-bg-glass-tertiary flex items-center justify-center">
                  {getClipIcon(clip)}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="absolute bottom-0 left-0 right-0 p-2">
                  ...
                  <div className="text-editor-text-secondary text-xs">
              ...
              <button
                ...
                className="absolute top-1 right-1 w-5 h-5 bg-editor-interactive-destructive text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-editor-interactive-destructive-hover border border-editor-border-secondary"
              >
                <X className="w-3 h-3" />
              </button>

              {/* Drag indicator */}
              <div className="absolute top-1 left-1 w-5 h-5 bg-editor-bg-glass-tertiary text-editor-text-tertiary rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity border border-editor-border-tertiary">
                <div className="w-2 h-2 bg-editor-text-tertiary rounded-full"></div>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* Footer info */}
      {clips.length > 0 && (
        <div className="px-3 py-2 border-t border-editor-border-tertiary bg-editor-bg-glass-secondary flex-shrink-0 backdrop-blur-xl">
